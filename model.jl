...
function trav(paras::Vector, num_rcv::Int64, rcv_lat::Vector, rcv_lon::Vector, rcv_dep::Vector, num_lay::Int64, thk::Vector, vp::Vector)
    src_lat = paras[1]
    src_lon = paras[2]
    src_dep = paras[3]
    src_latlon = LatLon(src_lat, src_lon)
    # compute the layer of source that located
    lay_dep = 0.0
    src_lay = 0
    for i=1:1:num_lay
    ...
    p0 = zeros(num_rcv)
    for ijk=1:1:num_rcv
        rcv_latlon = LatLon(rcv_lat[ijk], rcv_lon[ijk])
        x = distance(src_latlon, rcv_latlon, wgs84)
        x = x/1000.0
        # compute the layer of receiver that located
        lay_dep = 0.0
        rcv_lay = 0
        for i=1:1:num_lay
            lay_dep = lay_dep + thk[i]
            if lay_dep > rcv_dep[ijk]
                rcv_lay = i
                break
            end
        end
